...
async function createWASession(userId, phoneNumber) {
  ...
  // Fresh start — পুরনো session মুছে ফেলো
  ...
  console.log(`📱 WA pairing for: ${cleanPhone}`);
  ...
  const sock = makeWASocket({
    ...
    browser: ["Chrome (Linux)", "Chrome", "130.0.6723.116"],
  ...
  sock.ev.on("connection.update", ({ connection, lastDisconnect }) => {
    if (connection === "open") {
      ...
      console.log(`🔴 WA closed: user ${userId} sc=${sc}`);
  ...
  // ✅ Baileys pairing code: socket init হওয়ার পর, "open" এর আগেই request করতে হয়
